...
class mpiInfo
{
 public:
  ...
  inline void GridDecomposition(int _nPEx, int _nPEy, int nCellx , int nCelly)
  {
  ...
  inline void ExchangeBoundaryInfo(double* Solution, double* b)
  {
	sLOOP phiSend_n[s] = 0.;
	sLOOP phiSend_s[s] = 0.;
	tLOOP phiSend_e[t] = 0.;
  ...
  inline   void ParticleExchange( VI &ptcl_send_list , VI &ptcl_send_PE , particles &PTCL)
  {
    ...
    double* std_add_x  ;  std_add_x = new double[Np + 1];  for ( int i = 0 ; i < Np + 1 ; ++i ) std_add_x [i] = 0.;
    double* std_add_y  ;  std_add_y = new double[Np + 1];  for ( int i = 0 ; i < Np + 1 ; ++i ) std_add_y [i] = 0.;
    double* std_add_vx ;  std_add_vx = new double[Np + 1]; for ( int i = 0 ; i < Np + 1 ; ++i ) std_add_vx[i] = 0.;
    double* std_add_vy ; std_add_vy = new double[Np + 1]; for ( int i = 0 ; i < Np + 1 ; ++i ) std_add_vy[i] = 0.;
    ...
    PTCL.add(std_add_x , std_add_y, std_add_vx , std_add_vy, Np + 1 );
  ...
  inline void PEsum( double* field )
  {
    int tag    = 0;
  ...
  inline int pid(int i,int j) { return (i+1) + (j)*(nRealx+2); }  
