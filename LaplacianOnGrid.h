...
public:
  ...
  LaplacianOnGrid(double _x0   , double _x1,  double _y0, double _y1 , int ncell_x , int ncell_y, mpiInfo &myMPI )
  {
    ...
    x = new double[nField + 1];
    y = new double[nField + 1];
    phi = new double[nField + 1];
    b   = new double[nField + 1];
    Qval = new double[nField + 1];
    // x.resize(nField+1); 
    // y.resize(nField+1); 
    ...
    Acoef = new double*[nField+1];
    Jcoef = new int*[nField+1];
    rLOOP { Acoef[r] = new double[bandwidth + 1]; Jcoef[r] = new int[bandwidth + 1]; }
  ...
  inline void FormLS(mpiInfo &myMPI)
  {
  ...
  inline void ApplyBCs(int iMin, int iMax , int jMin, int jMax, VD &phiValues)
  {
    int count = 0;
  ...
  inline void ApplyNeumannBCs(int iMin, int iMax , int jMin, int jMax , int dirx , int diry)
  {
    for ( int i = iMin ; i <= iMax ; ++i )
      for ( int j = jMin ; j <= jMax ; ++j )
	{
    ...
    //unroll the above loop
    if ( Jcoef[p][1] == pid(i+dirx,j+diry) ) Acoef[ p ] [ 1 ] = -1.;
    if ( Jcoef[p][2] == pid(i+dirx,j+diry) ) Acoef[ p ] [ 2 ] = -1.;
    if ( Jcoef[p][3] == pid(i+dirx,j+diry) ) Acoef[ p ] [ 3 ] = -1.;
    if ( Jcoef[p][4] == pid(i+dirx,j+diry) ) Acoef[ p ] [ 4 ] = -1.;
    if ( Jcoef[p][5] == pid(i+dirx,j+diry) ) Acoef[ p ] [ 5 ] = -1.;
  ...
  inline void ParticlesOnMesh(particles &PTCL, mpiInfo &myMPI)
  {
    double hx, hy;
    double w[5];
    int    p[5];
	...
	if ( PTCL.active[k] == 1 )
      ...
      // unroll the above loop
      Qval[ p[1] ] +=  w[1] * PTCL.Qp;
      Qval[ p[2] ] +=  w[2] * PTCL.Qp;
      Qval[ p[3] ] +=  w[3] * PTCL.Qp;
      Qval[ p[4] ] +=  w[4] * PTCL.Qp;
     	  }
       }
  ...
  inline int pid(int i,int j) { return (i+1) + (j)*(nRealx+2); }  // Given i-j, return point ID.  Here i-j is the physical grid.
