...
inline void GS_or_Jacobi(int max_iter , double* RHS, double* Solution , mpiInfo &myMPI , int GSorJacobi, int &finalIterCount, int solution_size )
  {
    ...
    int        one  = 1;
		...
		double* SolutionNew;
		SolutionNew = new double[solution_size];
	...
	rLOOP
	  {
	    ...
	    // for ( int c = 2 ; c <= bandwidth ; ++c ) newval -=  Acoef[r][c] * Solution[Jcoef[r][c]];
			...
			// unroll the above loop for bandwidth = 5
			newval -=  Acoef[r][2] * Solution[Jcoef[r][2]];
			newval -=  Acoef[r][3] * Solution[Jcoef[r][3]];
			newval -=  Acoef[r][4] * Solution[Jcoef[r][4]];
			newval -=  Acoef[r][5] * Solution[Jcoef[r][5]];
